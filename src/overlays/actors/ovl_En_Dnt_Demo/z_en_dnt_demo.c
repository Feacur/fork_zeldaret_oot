        ...
        if (this->judgeTimer < 120) {
            ...
            switch (Player_GetMask(play)) {
                case PLAYER_MASK_SKULL:
                    if (!GET_ITEMGETINF(ITEMGETINF_GOT_STICK_UPGRADE)) {
                ...
                case PLAYER_MASK_TRUTH:
                    // @bug: should have been `ITEMGETINF_GOT_NUT_UPGRADE`
                    // the devs accidentally conflated `ITEMGETINF_1F`
                    if (!GET_ITEMGETINF(ITEMGETINF_GAVE_POACHERS_SAW) && (Player_GetMask(play) != PLAYER_MASK_SKULL)) {
