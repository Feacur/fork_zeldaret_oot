...
void BgGateShutter_Init(Actor* thisx, PlayState* play) {
    ...
    if ((GET_INFTABLE(INFTABLE_TALKED_TO_KAKARIKO_GUARD) || GET_EVENTCHKINF(EVENTCHKINF_HAS_MASTER_SWORD)) && (play->sceneId == SCENE_KAKARIKO_VILLAGE)) {
...
void func_8087828C(BgGateShutter* this, PlayState* play) {
    if (this->openingState == 1 && !GET_INFTABLE(INFTABLE_TALKED_TO_KAKARIKO_GUARD)) {
        this->unk_178 = 2;
